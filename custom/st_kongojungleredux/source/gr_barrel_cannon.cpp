...
void grAdventureBarrelCannon::attachGround(grYakumono* ground) {
...
void grAdventureBarrelCannon::startup(gfArchive* archive, u32 unk1, gfSceneRoot::LayerType layerType)
{
    ...
    anmChr = this->m_resFile.GetResAnmChr(2).ptr();
    if (anmChr != NULL) {
        this->animRespawnLength = anmChr->m_animLength;
    }
    if (this->m_modelAnims != NULL)
    {
        (this->m_modelAnims[0])->unbindNodeAnim(this->m_sceneModels[0]);
    }
    this->areaData.set(soAreaInstance::Shape_Rectangle, gfArea::Stage_Group_Gimmick_Normal, 0, 0, 0, 0, Vec2f(stageData->cannonAreaOffsetPos.m_x, stageData->cannonAreaOffsetPos.m_y), Vec2f(stageData->cannonAreaRange.m_x, stageData->cannonAreaRange.m_y));
    ...
    if (0 < simpleEffectData.m_id) {
        ...
        if (simpleEffectData.m_nodeIndex == 0) {
            this->m_effects[1].m_nodeIndex = this->getNodeIndex(0, "effect_locator");
        }
        else {
    ...
    if (0 < simpleEffectData.m_id) {
        ...
        this->m_effects[2].m_repeatFrame = simpleEffectData.m_repeatFrame;
        if (simpleEffectData.m_nodeIndex == 0) {
        ...
        this->m_effects[2].m_endFrame = simpleEffectData.m_endFrame;
        this->m_effects[2].m_offsetPos = Vec2f(0.0, 0.0);
        this->m_effects[2].m_scale = 1.0;
    }
    ...
    this->m_soundEffects[2].m_id = snd_se_ADVstage_common_08;
            ...
            switch(this->cannonPlayerInfos[i].state) {
                ...
                case PlayerState_Fire:
                    if (this->cannonPlayerInfos[i].frame <= this->cannonData->maxFrames) {
                        ...
                        if (this->attachedGround != NULL) {
                            this->attachedGround->m_gimmickMotionPath->setFrameUpdate(1.0);
                        }
...
void grAdventureBarrelCannon::update(float frameDelta)
{
    ...
    switch(this->cannonState) {
        ...
        case State_Respawn:
            this->m_modelAnims[0]->m_anmObjChrRes->SetFrame(this->animFrame);
            this->animFrame += frameDelta;
            if (this->animFrame >= this->animRespawnLength) {
                this->m_modelAnims[0]->unbindNodeAnim(this->m_sceneModels[0]);
                this->cannonState = State_Rest;
                this->animFrame = 0.0;
                this->enableArea();
...
void grAdventureBarrelCannon::updateMove(float frameDelta)
{
    ...
    if (this->cannonState != State_Respawn) {
        if ((pos.m_z > stageData->cannonActiveMinZ && pos.m_z < stageData->cannonActiveMaxZ) && !(stageData->isCannonSingleRider && this->isPlayerIn) && this->cooldownTimer <= 0.0) {
            this->enableArea();
            this->changeMatColAnim(0, 0);
        }
        else {
    ...
    switch (cannonEventInfo->m_kind) {
        case 0x2:
            if (stageData->isCannonShootStop) {
                ...
                if (this->attachedGround != NULL) {
                    this->attachedGround->m_gimmickMotionPath->setFrameUpdate(0);
                }
            }
            this->isRotate = true;
            for (int i = 0; i < NUM_PLAYERS; i++) {
...
void grAdventureBarrelCannon::presentShootEvent(int playerCannonIndex)
{
    ...
    this->cooldownTimer = stageData->cannonCooldownFrames;
